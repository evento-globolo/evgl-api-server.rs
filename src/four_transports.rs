#![forbid(unsafe_code)]
//! Serving the four avenues from `evgl-api`.
//!
...
//! |---|------|---------------------|
//! | 1 | `direct_read` | none: the web server reads Postgres itself through `evgl-lib-core` |
...
///
/// Bind `EVGL_API_MTLS_ADDR` and pass the listener. One task per
...
/// Open the JetStream context for avenue 4 from `EVGL_NATS_URL`.
///
/// # Errors
